...
- `services/`: Azure OpenAI, Azure AI Search, URL validation, citation, risk, credibility, and confidence services.
- `middleware/`: security, request tracing, validation, logging, rate limiting, JWT, and error handling.
- `configs/`: environment and Azure client configuration.
...
In production, Azure OpenAI, Azure AI Search, and JWT values are required by environment validation.
...
- Azure AI Search service with an index containing fields such as `title`, `url`, `content`, `summary`, `publisher`, and `publishedAt`.
- Azure App Service configured with the environment variables above.

The backend uses Azure OpenAI through LangChain and Azure AI Search through the official Azure SDK. In development, if Azure credentials are not configured, the service uses conservative local fallbacks so endpoints remain testable.
...
3. Set startup command to `node dist/server.js` or use `docker/appservice-startup.sh`.
4. Deploy with GitHub Actions, Azure CLI, or container deployment.
...
- Use Azure App Configuration or Key Vault for secrets.
- Use managed Redis in production.
- Configure Azure AI Search indexes with trusted source metadata.
- Add private networking for Azure OpenAI, Search, and Redis where available.
